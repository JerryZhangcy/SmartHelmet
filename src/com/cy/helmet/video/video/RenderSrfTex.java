...
public class RenderSrfTex {
    ...
    public void setWatermark(Watermark watermark) {
        ...
        mWatermarkImg = watermark.mBitmap;
    ...
    public void draw() {
        saveRenderState();
        {
            ...
            runFirstTimeSetUp();
            drawRecordingStream();//draw recording stream
            drawLivingStream();// draw live stream
    ...
    private void runFirstTimeSetUp() {
        ...
        if (mRecorder.firstTimeSetupLive()) {
            mRecorder.startLiveSwapData();
            mRecorder.makeLiveCurrent();
            initGL();
        }
    }

    private void drawRecordingStream() {

        if (mRecorder.isRecording()) {

            mRecorder.makeCurrent();

            VideoConfiguration recordConfig = VideoConfiguration.getRecordingConfig();
            int recordWidth = recordConfig.width;
            int recordHeight = recordConfig.height;
    ...
    private void drawLivingStream() {
        if (mRecorder.isLiving()) {

            mRecorder.makeLiveCurrent();

            VideoConfiguration liveConfig = VideoConfiguration.getLivingConfig();
            int liveWidth = liveConfig.width;
            int liveHeight = liveConfig.height;

            GLES20.glViewport(0, 0, liveWidth, liveHeight);
    ...
    private void drawWatermark() {
        ...
        mWatermarkImg = mWaterMark.getWatermarkBitmap();
        ...
        if (mWatermarkTextureId == -1) {
            mWatermarkTextureId = createTexture(mWatermarkImg);
        } else {
            updateTexture(mWatermarkTextureId, mWatermarkImg);
        }
