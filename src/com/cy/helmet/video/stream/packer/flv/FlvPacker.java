package com.cy.helmet.video.stream.packer.flv;
...
import static com.cy.helmet.video.stream.packer.flv.FlvPackerHelper.FLV_HEAD_SIZE;
import static com.cy.helmet.video.stream.packer.flv.FlvPackerHelper.FLV_TAG_HEADER_SIZE;
import static com.cy.helmet.video.stream.packer.flv.FlvPackerHelper.PRE_SIZE;
...
public class FlvPacker implements Packer, AnnexbHelper.AnnexbNaluListener {
    ...
    public void onVideoData(ByteBuffer bb, MediaCodec.BufferInfo bi) {
        long startAnalytics = System.currentTimeMillis();
        mAnnexbHelper.analyseVideoData(bb, bi);
        long finishAnalytics = System.currentTimeMillis();
        Log.e("YJQ", "analyticsTime: " + (finishAnalytics - startAnalytics));
    }
    ...
    public void onAudioData(ByteBuffer bb, MediaCodec.BufferInfo bi) {
        ...
        packData(buffer.array(), AUDIO, null, null, false);
    }
    ...
    public void onVideo(byte[] video, boolean isKeyFrame, byte[] sps, byte[] pps) {
        ...
        packData(video, packetType, sps, pps, isKeyFrame);
    }
    ...
    // Size limit of each file
    private long mPackedFileSize = 0;

    private void packData(byte[] data, int packetType, byte[] sps, byte[] pps, boolean isKeyFrame) {
        mPackedFileSize += data.length;
        long FILE_MAX_SIZE_BYTE = HelmetConfig.get().getMaxRecordFileLength() * 1024 * 1024;

        if (isKeyFrame && mPackedFileSize >= FILE_MAX_SIZE_BYTE && sps != null && pps != null && isKeyFrame) {
            //new file
            if (packetListener.onTransferFile()) {
                //写入Flv header信息
                writeFlvHeader();
                //写入Meta 相关信息
                writeMetaData();
                ...
                //TODO break video recording if create file failed
            }
        }
        ...
        if (AUDIO != packetType) {
            ...
            packetListener.onPacket(data, packetType);
        }
    }
